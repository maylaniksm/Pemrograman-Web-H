<div
    ...
    <form action="<?= BASEURL ?>/transaksi/update" method="POST">
        <input type="text" name="id" value="<?= $data['transaksi']['id'] ?>" hidden>
        <div style="margin-bottom: 15px;">
            <label for="nama" style="display: block; margin-bottom: 5px; font-weight: 600;">Nama</label>
            <input type="text" id="nama" name="nama" value="<?= $data['transaksi']['nama'] ?>"
                placeholder="Masukan nama pemesan"
                style="width: 100%; padding: 10px; border: 1px solid #ccc; border-radius: 4px;">
        </div>
        <div style="margin-bottom: 15px;">
            <label for="status" style="display: block; margin-bottom: 5px; font-weight: 600;">Status</label>
            <select type="text" id="status" name="status" value="<?= $data['transaksi']['status'] ?>"
                placeholder="Masukan nama pemesan"
                style="width: 100%; padding: 10px; border: 1px solid #ccc; border-radius: 4px;">
                <option value="proses" <?= $data['transaksi']['status'] === 'proses' ? 'selected' : '' ?> >proses</option>
                <option value="selesai" <?= $data['transaksi']['status'] === 'selesai' ? 'selected' : '' ?>>selesai</option>
            </select>
        </div>
        <div style="margin-bottom: 15px;">
            ...
            <select name="id_mobil" id="id_mobil" onchange="showTransaksiByMobil()"
                ...
                foreach ($data['mobils'] as $mobil) {
                    if ($mobil['id'] == $data['transaksi']['id_mobil']){
                        echo "<option value='{$mobil['id']}' selected>{$mobil['merek']} - Rp{$mobil['harga']}</option>";
                    } else {
        ...
        <div style="margin-bottom: 15px;">
            <label for="tarif" style="display: block; margin-bottom: 5px; font-weight: 600;">Tarif <span
                    id="label_tarif" style="font-weight: 400;"></span></label>
            <input type="text" id="tarif" name="tarif" value="<?= $data['transaksi']['tarif'] ?>" placeholder="Masukan nomor tarif"
                style="width: 100%; padding: 10px; border: 1px solid #ccc; border-radius: 4px; cursor: not-allowed;"
                readonly>
        </div>
        <div style="margin-bottom: 15px;">
            <label for="sewa_masuk" style="display: block; margin-bottom: 5px; font-weight: 600;">Sewa masuk</label>
            <input type="date" id="sewa_masuk" name="sewa_masuk" value="<?= $data['transaksi']['sewa_masuk'] ?>" onchange="showTransaksiByMobil()"
        ...
        <div style="margin-bottom: 15px;">
            ...
            <input type="date" id="sewa_keluar" name="sewa_keluar" value="<?= $data['transaksi']['sewa_keluar'] ?>" onchange="showTransaksiByMobil()"
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', showTransaksiByMobil());
</script>